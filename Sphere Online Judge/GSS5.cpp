	...
	for(cs = 1; cs <= test; cs++) 
    {
		...
		while(q--) 
        {
			...
			if(y1 < x2) 
            {
				...
				res += query(1, 0, n-1, y1+1, x2-1).sum + query(1, 0, n-1, x2, y2).pre;
			}
			else 
            {
				res = query(1, 0, n-1, x2, y1).max;
				res = _max(res, _max(query(1, 0, n-1, x1, x2-1).suf + query(1, 0, n-1, x2, y2).pre,query(1, 0, n-1, x1, y1).suf + query(1, 0, n-1, y1+1, y2).pre));
			}
			printf("%d\n", res);
		}
	}
